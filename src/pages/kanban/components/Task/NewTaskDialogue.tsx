...
import { IList } from "../../models/atoms"
...
const NewTaskDialogue = ({ listId, tasks, index }: IList) => {
  ...
  const submit = ({ name, description }: INewTaskForm) => {
    const newTask = { id: uuidv4(), name: name, description: description }
    try {
      setTasks((allTasks) => ({
        ...allTasks,
        [listId]: [newTask, ...allTasks[listId]],
      }))
      setValue("name", "")
  ...
  return (
    <Form onSubmit={handleSubmit(submit)}>
      ...
      <Submit submitting={submitting}>{t("Add")}</Submit>
    </Form>
  )
}
